...
/// pass the result to `magnitudes_to_chromagram_sequence` for each tuning.
pub fn compute_frame_magnitudes(audio: &[f32]) -> Vec<Vec<f32>> {
...
/// Maps pre-computed FFT magnitudes to a chromagram sequence for the given
/// tuning reference. Cheap to call repeatedly with different `a4_freq` values.
pub fn magnitudes_to_chromagram_sequence(
